...
func (con GoodsController) Index(c *gin.Context) {
	...
	//判断最后一页有没有数据 如果没有跳转到第一页   TODO 没有理解具体含义
	if len(goodsList) > 0 {
...
// ChangeGoodsImageColor 修改商品图库关联的颜色
...
func (con GoodsController) Delete(c *gin.Context) {
	...
	if err != nil {
		...
		//获取上一页
		prevPage := c.Request.Referer() // 删除数据成功之后，返回到当前页
		if len(prevPage) > 0 {
